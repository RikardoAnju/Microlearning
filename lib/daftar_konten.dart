...
class DaftarKonten extends StatelessWidget {
  ...
  DaftarKonten({required this.lessonId});
  ...
  Future<List<Map<String, dynamic>>> fetchKonten() async {
    try {
      var kontenSnapshot = await _firestore
          .collection('konten')
          .where('lessonId', isEqualTo: lessonId) // Mengambil data berdasarkan lessonId
          .get();
  ...
  Widget buildContentCard(Map<String, dynamic> content) {
    return Card(
      ...
      color: const Color(0xFFFFFD55), // Warna latar belakang kuning
      child: Padding(
